...
export class ThreatDetector {
  ...
  private detectPattern(url: string, userAgent: string): ThreatPattern | null {
    ...
    for (const pattern of this.threatPatterns) {
      ...
      if (matchCount > 0) {
        console.log(
          `🚨 Threat pattern detected: ${pattern.type}, indicators: ${pattern.indicators.filter((i) => searchText.includes(i.toLowerCase()))}`
        );
        return pattern;
      }
    }
  ...
  private calculateRiskScore(
    ...
    const recentActivityPenalty =
      timestamp - profile.lastSeen < 300000 ? 20 : 0;
   ...
   */
  cleanup(retentionHours: number = 168): void {
    // 7 days default
